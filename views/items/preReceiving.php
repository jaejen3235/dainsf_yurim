    ...
    <div class='search-wrapper'>
        <div class='search-box'>
            ...
            <div class='button-box'>
                <input type='button' class='btn-large orange' id='btnOpenModal' value='자재 입고 등록' />
...
<?php
include "./views/modal/modalIncomingInspection.php";
?>
    ...
    try {
        ...
        if(btnOpenModal) {
            btnOpenModal.addEventListener('click', function() {
                openModal('modalIncomingInspection', 1000, 660);
            });
        }
    } catch(e) {}
...
const generateTableContent = (data) => {
    ...
    return data.data.map(item => `
        <tr>
            ...
            <td class='center'>
                <input type='button' class='btn-small primary' value='수입검사' onclick='incomingInspection(${item.uid})' />
                <input type='button' class='btn-small danger' value='삭제' onclick='deleteItemsIn(${item.uid})' />
            </td>
        </tr>
    `).join('');
};

const incomingInspection = (uid) => {
    getter(uid);
    openModal('modalIncomingInspection', 1000, 570);
}

const deleteItemsIn = (uid) => {
    if(confirm('해당 데이터를 삭제하시겠습니까?')) {
        const formData = new FormData();
        formData.append('controller', 'mes');
        formData.append('mode', 'deleteItemsIn');
...
const deleteSelected = () => {
	...
	if(confirm("선택하신 DATA를 삭제하시겠습니까? 삭제 후에는 복구가 불가능합니다")) {
        ...
        formData.append('table', 'mes_items_inout');
