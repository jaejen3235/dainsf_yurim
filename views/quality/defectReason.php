<div class='main-container'>
    <div class='title-wrapper'>불량 사유 관리</div>
        <div class='search-wrapper'>
            ...
            <div class='button-box'>
                <input type='button' class='btn-large orange' id='btnOpenModal' value='불량사유 등록' />
...
<?php
include "./views/modal/modalRegisterDefectReason.php";
?>
...
const getDefectReasonList = async ({
    ...
    try {
        ...
        getPaging('mes_defect_reason', 'uid', where, page, per, block, 'getDefectReasonList');
    } catch (error) {
...
const generateTableContent = (data) => {
    ...
    return data.data.map(item => `
        <tr>
            ...
            <td class='center'>
                <input type='button' class='btn grey' value='수정' onclick='modfiyDefectReason(${item.uid})' />
                <input type='button' class='btn' value='삭제' onclick='deleteDefectReason(${item.uid})' />
            </td>
        </tr>
    `).join('');
};

const modfiyDefectReason = (uid) => {
    getter(uid);
    openModal('modalRegisterDefectReason', 600, 330);
}

const deleteDefectReason = (uid) => {
    if(confirm('해당 데이터를 삭제하시겠습니까?')) {
        const formData = new FormData();
        formData.append('controller', 'mes');
        formData.append('mode', 'deleteDefectReason');
...
const deleteSelected = () => {
	...
	if(confirm("선택하신 DATA를 삭제하시겠습니까? 삭제 후에는 복구가 불가능합니다")) {
        ...
        formData.append('table', 'mes_defect_reason');
