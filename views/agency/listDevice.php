    ...
    <div class='title-wrapper'>기기 관리</div>
        <div class='search-wrapper'>
            ...
            <div class='button-box'>
                <input type='button' class='btn-large orange' id='btnOpenModal' value='기기 등록' />
    ...
    try {
        ...
        if(btnOpenModal) {
            btnOpenModal.addEventListener('click', function() {
                location.href = `?controller=agency&action=registerDevice`;
            });
        }
    } catch(e) {}
...
const generateTableContent = (data) => {
    ...
    return data.map(item => `
        <tr>
            ...
            <td class='center'><img src='../attach/device/${item.thumb1}' style='width:100px' /></td>
            ...
            <td class='center'>
                <input type='button' class='btn grey' value='수정' onclick='modifyDevice(${item.uid})' />
                <input type='button' class='btn' value='삭제' onclick='deleteDevice(${item.uid})' />
            </td>
        </tr>
    `).join('');
};
...
const modifyDevice = (uid) => {
    location.href = `?controller=agency&action=registerDevice&uid=${uid}`;
}
...
const deleteDevice = (uid) => {
    if(confirm('해당 데이터를 삭제하시겠습니까?')) {
        ...
        formData.append('mode', 'deleteDevice');
...
const deleteSelected = () => {
	...
	if(confirm("선택하신 DATA를 삭제하시겠습니까? 삭제 후에는 복구가 불가능합니다")) {
        ...
        formData.append('table', 'sk_devices');
